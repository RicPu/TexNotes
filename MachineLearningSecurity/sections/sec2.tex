...
A poisoning attack is a training-time attack where an adversary injects malicious data into a model's training pipeline to corrupt the learned parameters. The primary goal is to compromise the system's normal operation. Two distinct high-level objectives define these attacks:

\begin{itemize}
    \item \textit{Indiscriminate Poisoning (Availability Attack)}: The goal is to cause a general degradation in overall model performance, degrading the system's availability.
    \item \textit{Targeted Poisoning (Integrity Attack)}: The goal is to create a specific, hidden vulnerability to allow subsequent intrusion on chosen inputs without affecting general performance, attacking the system's integrity.
\end{itemize}
...
\begin{itemize}
    \item \textit{Inner Problem (Follower)}: The legitimate training algorithm minimizes its loss on the poisoned training set, which includes the attacker's rogue point \((x_c, y_c)\).

          \begin{equation}
              w^*(x_c) \in \arg\min_w \mathcal{L}(D_\text{tr} \cup \{(x_c, y_c)\}, w)
          \end{equation}

    \item \textit{Outer Problem (Leader)}: The attacker searches for the optimal rogue point that, when used in the inner problem, forces the resulting model \(w^*\) to perform the worst on a clean validation set.
...
Directly solving this nested problem is computationally hard. Common approaches include replacing the inner loop with its Karush-Kuhn-Tucker (KKT) equilibrium conditions or using implicit differentiation to compute hypergradients is tractable for convex models like SVM and linear regression. For non-convex deep neural networks (DNNs), methods like unrolled optimization are used, which approximate the inner solution by back-propagating through a finite number of training steps. In some linear model settings, optimal indiscriminate attacks have been shown to reduce to simpler, highly efficient heuristics like maximizing class-conditional data density.

\paragraph{Specialized Poisoning Attack Variants.} Beyond accuracy, poisoning attacks can target other critical system properties. Poisoning Attacks on Algorithmic Fairness introduce or amplify discriminatory bias against specific groups. This is achieved via a gradient-based optimization framework that crafts points to maximize the disparity in model performance across groups. Notably, attacks optimized on a substitute model can transfer to a black-box target model. Sponge Poisoning aim to drain battery and cause delays in resource-constrained systems. The attacker manipulates the training to create a model with reduced activation sparsity. This forces hardware accelerators to perform more computations during inference for any input, increasing energy use and latency without affecting accuracy.
...
A backdoor poisoning attack is a type of security threat in amchine learning where an adversary manipulates a model during its training phase. The goal is to implant a secret backdoor, which is a specific pattern or trigger. The compromised model behaves normally on regular, clean inputs but performs an adversary-selected, often malious, behavior wen it detects the triger at test time.
...
\begin{itemize}
    ...
    \item \textit{Hidden Triggers}: A key limitation of BadNets is that the trigger is visible in the poisoned training data, making detection possible. To overcome this, Saha et al. introduced Hidden Trigger Backdoor Attacks. Their method uses clean, correclty labeled images during poisoning. It adds a small, optimized perturbation to these images, creating a feature collision in the model's internal representation. This collision causes the perturbed clean image to be classified as the target class, while the attacker uses a different, secret trigger to activate the backdoor during deployment.
...
\begin{enumerate}
    \item \textit{Training Data Sanitization}: This approach is based on the premise that poisoned data points often behave as satistical outliers. Core methods include:
          ...
          \begin{itemize}
              ...
              \item \textit{Label Sanitization}: Particularly useful against label-flipping attacks, this technique audits data labels for consistency. A common method involves checking a data poin's \(k\) nearerst neighbors and performing a majority vote to correct potentially flipped labels. Its primary limitation is that it is generally ineffective against clean-label attacks, where the poison lies in the data features, not the labels.
          \end{itemize}
    ...
    \begin{itemize}
        \item \textit{Activation Clustering}: A seminal method for detectin backdoors. It focuses on a suspected target class, extracts deep-layer activation vectors for all training sampels in that class, and performs dimensionality reduction followed by K-means clustering (with \(k = 2\)). The separation into two distinct clusters often reveals the poisoned subset. A key advantage is that it operates without requiring a trusted, clean dataset.
        \item \textit{Fine-Pruning}: This method exploits the observation that backdoors often rely on a small set of dormant neurons that are highly active only for triggered inputs. The process inveolves pruning neurons with the lowest average actiavtion on clean data and then fine-tuning the model to recover accuracy. An adaptive attacker can potentially circumvent this by designing triggers that also activate neurons used for clean tasks.
    \end{itemize}
    ...
    TABOR inorporates regularization terms to constrain the trigger to be smooth and small, preventing the detection of false, meaningless patterns.
    \item \textit{Test Data Sanitization}: This use-side defense assumes the model is arleady poisoned and aims to neutralize the trigger before inference. FEBRUUS is a method designed for image data. It first generates a heatmap to identify the image regions most influenced on the model's prediction. In a poisoned image, this region typically corresponds to the trigger. It then removes the identified region and uses inpainting techniqeus to reconstruct a clean version of the image for classification.
\end{enumerate}